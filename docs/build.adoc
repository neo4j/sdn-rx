...
We are gonna track the current release cadence of Java.
...
* While there is an increase of projects started with JDK 11 (which is Oracles current LTS release of Java),
  ...
  We don't to lose them as users or customers right from the start.

All examples (`/examples`) and also all benchmarking (`/benchmarks`) will be compiled and run with the latest Java release, though.
...
In short, Docker is a tool that helps you running lightweight software images using OS-level virtualization as so called containers.
...
A required step is to open a browser and goto http://localhost:7474 after you started the database and
...
We also use https://jqassistant.org[jQAssistant], a Neo4j tool, to verify some aspects of our architecture.
