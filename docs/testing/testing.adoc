...
`org.neo4j.springframework.data:spring-data-neo4j-rx-spring-boot-test-autoconfigure`. Include the following dependency in your project setup
...
Both slices provides all the necessary infrastructure for tests using Neo4j: A driver bean, a transaction manager, a client, a template and declared repositories,
...
If you annotate at test with it the test becomes `@Transactional`.
...
This is the default. See the <<imperative-template-example, imperative template example test>>.
