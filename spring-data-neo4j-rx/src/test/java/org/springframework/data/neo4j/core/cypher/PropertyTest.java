...
class PropertyTest {
	...
	void preconditionsShouldBeAsserted() {
		...
		assertThatIllegalArgumentException().isThrownBy(() -> Property.create(Node.create("a").as("s"), null))
			.withMessage(
				expectedMessage);
		assertThatIllegalArgumentException().isThrownBy(() -> Property.create(Node.create("a").as("s"), "\t"))
			.withMessage(
				expectedMessage);
	}
}
